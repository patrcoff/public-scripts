...
def extract_pod(input_filename):
    ...
    for i in range(1, filelimit + 1): #in this loop we shouold be checking if order num can be found at all in any of the pages (or even just not try to do multi-page ones)
        ...
        #------------------------------------------------------------------------COULD USE RE.FINDALL LIST RESULT'S LENGTH IN VALIDATION STEPS?
        try:
            del_notes = []
            for i in texta:
                if (len(i) == 8 and re.match(r"(\d{6}-\d{1})",i)) or (len(i) == 6 and re.match(r"(\d{6})",i)):
                    del_notes.append(i)
                else:
                    pass
        except:
            ...
            pass
        #then within del_notes filter as below:
            #how many results for 6 digit numbers?
            #what type of POD is this?
                #if picklist, are there 2 matching results for xxxxxx-x
                #if 6 digit, is order no etc in text near result
                #is - and or 1 in next position of array
                #compare text and texta results of filters!!!
        print(del_notes)
    ...
    try:
        if len(del_notes) == 1:
            return del_notes[0]
        elif len(del_notes) == 2 and del_notes[0] == del_notes[1] and "PICKING LIST" in text: #valid pick list:
            return del_notes[0]
    except:
