#had to compile using python 3.6 environment due to issue wih pyinstaller and python 3.10 and Paul's computer (something to do with openssl)
#had to edit the code to directly import messagebox (not sure why but this didn't work in python 3.6 otherwise)
#to do:
...
def addOrder():
    ...
    if len(ord) == 8 and ord not in selected_orders:
        ...
        try:
            ...
            input_order.delete(0,'end')#1.0,tk.END
            messagebox.showinfo(title="Error!",message="You entered invalid text!")
            return None
            #with an except clause in case it goes wrong
    else:
        messagebox.showinfo(title="Error!",message="You entered an invalid or duplicate order number!")
def processFile():
    global file
    if not exists(file):
        messagebox.showinfo(title="Error!",message="Please select an input file!")
        return None
    filename = file
    orders = []
    line_string = ''
    ...
    for order in orders: #order[0] = Header, order[1] = D ...
            ...
            for line in order:#for each H and D row in # i:
                ...
                for field in line:
                    if field != "hubdoc.cityflooringcentre.ipzg63nj@app.hubdoc.com" and field != "ap@sig.ie": #added logic to remove city flooring and HHI's duplicate email address
                        ...
                        messagebox.showinfo(title="Please note:",message="Removed duplicate email address from order.")
        ...
        #count commas and check
            ...
            question = messagebox.askquestion('Add Entry?', Q)
            if question == "yes":
...
def mayo():
    ...
    if exists("./routes_override.txt"):
        with open("./routes_override.txt", 'r+') as override:
            ...
            if len(lines) > 1:
                messagebox.showinfo(title="Error",message="Routes override file contains more than one row!")
                return None
            elif len(lines) == 1:
                ...
                messagebox.showinfo(title="Info",message="Routes override file is empty!")
        #messagebox.showinfo(title="Debug",message="This file exists!")
    else:
        messagebox.showinfo(title="Info",message="Proceeding with default routes.")
    #add json file for editing routes post compilation
    orders = []
    line_string = ''
    #print(file[-3:])
    #messagebox.showinfo(title="Please note:",message="")
    if filename[-3:] != 'csv' and filename[-3:] != 'CSV':
        #print("You crazy fool! I won't let you!")
        messagebox.showinfo(title="Error!",message="Source file doesn't quite look right, not a \'csv\' file - try again!")
        #sys.exit()
        return None
    with open(filename, 'r+') as read_obj:#file opened for reading
        csv_reader = reader(read_obj)
        for row in csv_reader:
            ...
            if row[0] == 'H':
                ...
                messagebox.showinfo(title="Error!",message="Source file doesn't quite look right, some lines don't start with \'H\' or \'D\' - try again!")
                #sys.exit()
                return None
    output = [] #new array for storing lines to be written to file
    for i in orders:
        ...
        if i[0][4] == "MAYO/SLIGO": #HERE WE NEED TO ADD LOGIC FOR IF INAPPROPRIATE COMMMAS HAVE BEEN ENTERED BEFORE ROUTE - I.E. IF I[0][4] NO IN ROUTES[] THEN TELL USER!!! - actually this never happens so never mind
            ...
            messagebox.showinfo(title="Error!",message="Please check source file for errors: route not recognised: "+i[0][4])
    ...
    messagebox.showinfo(title="Success!",message="Successfully filter Mayo/Sligo entries.")
#------------------------------------------------------------------------------
